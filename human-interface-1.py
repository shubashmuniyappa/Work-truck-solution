...
def process_invoice(url: str):
    ...
    try:
        ...
        return {
            ...
            "source_url": url  # Store the source URL for display
        }
...
if st.session_state.invoices_to_review:
    if st.session_state.current_invoice_idx < len(st.session_state.invoices_to_review):
        ...
        st.markdown(f"**File Path:** `{current_invoice['final_data'].get('file_path', 'N/A')}`")
        ...
        if current_invoice['final_data'].get('error'):
            ...
            with col1:
                ...
                st.markdown(f'<iframe src="{current_invoice["source_url"]}" width="100%" height="600"></iframe>', unsafe_allow_html=True)
            ...
            with col2:
                ...
                with st.form(key=form_key):
                    ...
                    # Iterate through the fields defined in the Pydantic model
                    ...
                    if submit_button:
                        ...
                        final_output_for_reviewed['final_data'].pop("confidence_scores", None)
