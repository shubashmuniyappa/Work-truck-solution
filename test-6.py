...
For each field, if the information is not explicitly found in the document, leave the string fields empty ("") and numerical fields as 0.0, unless specific fallback rules apply.
...
- **make**:
    - Look for the keyword "make".
    - If "make" keyword is not present, identify the car manufacturer/company mentioned anywhere else in the invoice (e.g., Ford, Chevrolet, Toyota).
    - If neither is found, leave empty ("").

- **model**:
    - Look for the keyword "model".
    - Extract the associated model name or number.
    - If not present, leave empty ("").

- **year**:
    - Look for the keyword "year".
    - If "year" keyword is not present, extract the year from the "invoice_date".
    - If neither is found, leave empty ("").
